...
from quantitativetrader.trader.ui import QtCore, QtGui, QtWidgets
from quantitativetrader.trader.ui.widget import (
    BaseCell,
    EnumCell,
    MsgCell,
    TimeCell,
...
from quantitativetrader.app.cta_strategy.strategies import *
from ..base import (
    APP_NAME,
    EVENT_CTA_LOG,
    EVENT_CTA_STOPORDER,
...
class CtaManager(QtWidgets.QWidget):
    ...
    def init_ui(self):
        ...
        add_button = QtWidgets.QPushButton("添加机器人")
        add_button.setStyleSheet("QPushButton{color:black}"
        ...
        start_button.setStyleSheet("QPushButton{color:black}"
                                       "QPushButton:hover{color:grey}"
        ...
        stop_button.setStyleSheet("QPushButton{color:black}"
                                       "QPushButton:hover{color:grey}"
        ...
        clear_button.setStyleSheet("QPushButton{color:black}"
                                       "QPushButton:hover{color:grey}"
                                       "QPushButton{background-color:rgb(251, 188, 5)}"
                                       "QPushButton{border:2px}"
                                       "QPushButton{border-radius:10px}"
    ...
    def update_class_combo(self):
        """"""
        for name in self.cta_engine.get_all_strategy_class_names():
            self.class_combo.addItem(NAME_DISPLAY_MAP[name])
    ...
    def add_strategy(self):
        ...
        parameters, parameters_map = self.cta_engine.get_strategy_class_parameters(class_name)
        editor = SettingEditor(parameters, parameters_map, class_name=class_name)
        n = editor.exec_()
...
class StrategyManager(QtWidgets.QFrame):
    ...
    def __init__(
        ...
        for key, value in self._data["variables"].items():
            if key == "inited":
                self.variables_map["inited"] = "机器人初始化"
            elif key == "trading":
                self.variables_map["trading"] = "机器人启动"
            elif key == "pos":
                self.variables_map["pos"] = "仓位"

        for key, value in globals().items():
            if key == self._data["class_name"]:
                if hasattr(value, "parameters_map"):
                    self.parameters_map = value.parameters_map
                else:
                    if hasattr(value, "parameters"):
                        for key in self._data["parameters"].keys():
                            self.parameters_map[key] = key
                if hasattr(value, "variables_map"):
                    self.variables_map.update(value.variables_map)
    ...
    def init_ui(self):
        ...
        self.init_button.setStyleSheet("QPushButton{color:black}"
                                       "QPushButton:hover{color:grey}"
                                       "QPushButton{background-color:rgb(66, 133, 244)}"
                                       "QPushButton{border:2px}"
                                       "QPushButton{border-radius:10px}"
        ...
        self.remove_button.setStyleSheet("QPushButton{color:black}"
                                       "QPushButton:hover{color:grey}"
                                       "QPushButton{background-color:rgb(251, 188, 5)}"
                                       "QPushButton{border:2px}"
                                       "QPushButton{border-radius:10px}"
        ...
        market = "现货交易" if vt_symbol.split(".")[-1] == "BINANCE" else "合约交易"

        label_text = (
            f"{strategy_name} - {NAME_DISPLAY_MAP[class_name]} - {vt_symbol.split('.')[0].upper()} - {market}"
        )
        label = QtWidgets.QLabel(label_text)
        label.setAlignment(QtCore.Qt.AlignCenter)

        self.parameters_monitor = DataMonitor(self._data["parameters"], self.parameters_map)
        self.variables_monitor = DataMonitor(self._data["variables"], self.variables_map)
...
class DataMonitor(QtWidgets.QTableWidget):
    ...
    def __init__(self, data: dict, map: dict):
        """"""
...
class SettingEditor(QtWidgets.QDialog):
    ...
    def __init__(
        self, parameters: dict, parameters_map: dict, strategy_name: str = "", class_name: str = ""
    ):
        """"""
    ...
    def init_ui(self):
        ...
        if self.class_name:
            self.setWindowTitle("设置机器人参数")
            button_text = "添加"
            parameters = {"strategy_name": "", "vt_symbol": ""}
            parameters_map = {"strategy_name": "机器人名称", "vt_symbol": "交易对"}
            parameters.update(self.parameters)
            parameters_map.update(self.parameters_map)
        else:
            self.setWindowTitle(f"参数编辑：{self.strategy_name}")
            button_text = "确定"
            parameters = self.parameters
            parameters_map = self.parameters_map
