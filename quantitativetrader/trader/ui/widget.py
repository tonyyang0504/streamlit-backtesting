"""
Basic widgets for Quantitative Trading Robots.
"""
...
from ..setting import SETTING_FILENAME, SETTINGS, SETTINGS_MAP


COLOR_LONG = QtGui.QColor(52, 168, 83)
COLOR_SHORT = QtGui.QColor(234, 67, 53)
COLOR_BID = QtGui.QColor(52, 168, 83)
COLOR_ASK = QtGui.QColor(234, 67, 53)
...
class BaseMonitor(QtWidgets.QTableWidget):
    """
    Monitor data update in Quantitative Trading Robots.
    """
...
class TickMonitor(BaseMonitor):
    ...
    headers = {
        ...
        # "name": {"display": "交易对", "cell": BaseCell, "update": True},
...
class OrderMonitor(BaseMonitor):
    ...
    headers: Dict[str, dict] = {
        "orderid": {"display": "委托号", "cell": BaseCell, "update": False},
        "reference": {"display": "机器人", "cell": BaseCell, "update": False},
        "symbol": {"display": "交易对", "cell": BaseCell, "update": False},
        # "exchange": {"display": "交易所", "cell": EnumCell, "update": False},
...
class ConnectDialog(QtWidgets.QDialog):
    ...
    def init_ui(self) -> None:
        """"""
        market = "现货交易市场" if self.gateway_name == "BINANCE" else "合约交易市场"
        
        self.setWindowTitle(f"连接{market}")
        ...
        for field_name, field_value in default_setting.items():
            ...
            form.addRow(f"{field_name}", widget)
...
class TradingWidget(QtWidgets.QWidget):
    ...
    def init_ui(self) -> None:
        ...
        bid_color = "rgb(52, 168, 83)"
        ask_color = "rgb(234, 67, 53)"
...
class ContractManager(QtWidgets.QWidget):
    ...
    def init_ui(self) -> None:
        ...
        self.button_show.setStyleSheet("QPushButton{color:black}"
                                       "QPushButton:hover{color:grey}"
                                       "QPushButton{background-color:rgb(251, 188, 5)}"
                                       "QPushButton{border:2px}"
                                       "QPushButton{border-radius:10px}"
...
class AboutDialog(QtWidgets.QDialog):
    """
    About Quantitative Trading Robots.
    """
    ...
    def init_ui(self) -> None:
        """"""
        self.setWindowTitle(f"关于Quantitative Trading Robots")

        text = f"""


            Quantitative Trading Robots by Alpex


                           version: {quantitativetrader.__version__}
            """
...
class GlobalDialog(QtWidgets.QDialog):
    ...
    def init_ui(self) -> None:
        ...
        for field_name, field_value in settings.items():
            ...
            form.addRow(f"{field_display_name}", widget)
            self.widgets[field_name] = (widget, field_type)
    ...
    def update_setting(self) -> None:
        ...
        box = QtWidgets.QMessageBox(
            ...
            "注意",
            "全局配置的修改需要重启Quantitative Trading Robots后才会生效！"
        )

        box.setStandardButtons(QtWidgets.QMessageBox.Ok)
        box.button(QtWidgets.QMessageBox.Ok).setText("确定")
        box.setStyleSheet("QPushButton{color:black}"
                                       "QPushButton:hover{color:grey}"
                                       "QPushButton{background-color:rgb(251, 188, 5)}"
                                       "QPushButton{border:2px}"
                                       "QPushButton{border-radius:10px}"
